...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  new make(Compiler fan.compiler)
    : super(fan.compiler)
  {
    resolved[fan.compiler.pod.name] = fan.compiler.pod
  }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  override Void run()
  {
    ...
    if (fan.compiler.input.isScript)
    {
      resolved.each |pod|
      {
        if (pod !== fan.compiler.pod && pod.name != "sys")
          fan.compiler.input.depends.add(Depend("$pod.name $pod.version"))
      }
    }
  }
  ...
  **
  private Void resolveImports(CompilationUnit unit)
  {
    ...
    unit.usings.each |Using u|
    {
      ...
      // check that this podName was in the fan.compiler's
      // input dependencies
      checkUsingPod(this, podName, u.location)

      // don't allow a using my own pod
      if (u.typeName == null && u.podName == fan.compiler.pod.name)
  ...
  **
  private Void resolveImportedTypes(CompilationUnit unit)
  {
    ...
    addAll(types, fan.compiler.pod.types)
